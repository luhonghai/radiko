...
$NDK/build/tools/make-standalone-toolchain.sh --toolchain=x86-4.6 --platform=android-9 \
...
for version in x86; do
	...
	case "$version" in
		x86)
			...
			ABI="x86"
            if [ ! -z "$SSL" ]; then
                EXTRA_LDFLAGS="-L$SSL_LD/libs/x86"
                CFLAGS="-I$SSL/include"
                SSL_OBJS=`find $SSL_LD/obj/local/x86/objs/ssl $SSL_LD/obj/local/x86/objs/crypto -type f -name "*.o"`
            fi
			;;
	esac
	DEST="$DEST/$ABI"
	...
	./configure $FLAGS --extra-cflags="$CFLAGS $EXTRA_CFLAGS" --extra-ldflags="$EXTRA_LDFLAGS" --extra-cflags='-mincoming-stack-boundary=4' | tee $DEST/configuration.txt
	[ $PIPESTATUS == 0 ] || exit 1
	make clean
	make -j4 || exit 1
	make install || exit 1
